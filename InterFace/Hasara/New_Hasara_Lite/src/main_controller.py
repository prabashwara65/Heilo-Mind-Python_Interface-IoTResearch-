...
MOBILE_TABLE_NAME = "userAppVisits"
...
class SolarPredictionHandler:
    ...
    def get_last_24h_prediction(self, request_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve the last 24-hour prediction from DynamoDB
        Used for mobile requests with requestId starting with "EN"
        """
        try:
            ...
            response = self.solar_table.scan(
                FilterExpression=Attr('timestamp').gt(twenty_four_hours_ago)
            )
            
            items = response.get('Items', [])
            
            if not items:
                logger.warning("No predictions found in last 24 hours")
                return None
            
            # Sort by timestamp (newest first) and get the latest
            latest_prediction = sorted(items, key=lambda x: x.get('timestamp', 0), reverse=True)[0]
            
            logger.info(f"Retrieved latest prediction from DynamoDB: {latest_prediction.get('total_energy_formatted', 'N/A')}")
...
class MobileRequestHandler:
    """
    Handles mobile requests with requestId starting with "EN"
    """
    
    def __init__(self, solar_handler: SolarPredictionHandler):
        self.solar_handler = solar_handler
        self.dynamodb = boto3.resource('dynamodb', region_name=REGION)
        self.mobile_table = self.dynamodb.Table(MOBILE_TABLE_NAME)
        
    def handle_mobile_request(self, request_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        Handle mobile request and return last 24h prediction
        """
        logger.info(f"📱 Processing mobile request with ID: {request_id}")
        
        # Get last 24h prediction
        prediction_data = self.solar_handler.get_last_24h_prediction(request_id)
        ...
        if prediction_data and prediction_data.get('prediction'):
            ...
            response = {
                ...
                'status': 'success',
                'data': {
                    'total_energy_kwh': prediction.get('total_energy_kwh', 0),
                    'total_energy_formatted': prediction.get('total_energy_formatted', 'N/A'),
                    'timestamp': prediction.get('timestamp', time.time()),
                    'timestamp_str': prediction.get('timestamp_str', datetime.now().isoformat()),
            ...
            logger.info(f"✅ Mobile response prepared for {request_id}: {response['data']['total_energy_formatted']}")
            
            # Optionally log to mobile table
            try:
                self.mobile_table.put_item(Item={
                    'requestId': request_id,
                    'timestamp': time.time(),
                    'response': response
                })
            except Exception as e:
                logger.error(f"Failed to log to mobile table: {e}")
...
class IoTModelProcessor:
        ...
        default_config = {
            "aws_iot": {
                ...
                "subscribe_topics": [
                    ...
                    "raspberrypi/request"  # Mobile requests
                ],
            ...
            "prediction_interval": 3600,  # Seconds between auto-predictions (1 hour)
            "auto_predict": True,
    ...
    def handle_mobile_request(self, payload: Dict[str, Any]):
        """
        Handle mobile request - look for requestId starting with "EN"
        """
        try:
            ...
            # Check if requestId starts with "EN" (case insensitive)
            if request_id.upper().startswith('EN'):
                logger.info(f"✅ Valid mobile request pattern detected")
                ...
                logger.debug(f"Ignoring non-mobile request ID: {request_id}")
    ...
    def send_prediction_request(self, sensor_data: list, device_id: str = "Arduino_RPi") -> str:
        ...
        try:
            ...
            logger.info(f"   Sensor data: 24 readings prepared")
    ...
    def on_arduino_data(self, data: Dict[str, float]):
        ...
        """
        # You can add logic here to trigger predictions based on conditions
        pass
    ...
    def start(self):
        ...
        logger.info("   - Mobile requests (raspberrypi/request) with EN* IDs")
        logger.info("=" * 60)
...
def test_mobile_request():
    ...
    # Simulate a mobile request with EN requestId
    test_request = {
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='IoT Model Processor with Arduino and Mobile')
